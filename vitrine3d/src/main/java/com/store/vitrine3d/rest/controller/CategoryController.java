...
public class CategoryController {
    ...
    public List<CategoryResponse> findPublic() {
        return categoryService.findPublic().stream().map(CategoryResponse::from).toList();
    }
    ...
    public List<CategoryResponse> findByStore(@PathVariable UUID storeId) {
        return categoryService.findByStore(storeId).stream().map(CategoryResponse::from).toList();
    }
    ...
    public ResponseEntity<CategoryResponse> create(@Valid @RequestBody CategoryCreateRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(CategoryResponse.from(categoryService.create(request.getName())));
    }
